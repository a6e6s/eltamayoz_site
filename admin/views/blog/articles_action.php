...
if (isset($_GET['id'])) {
    ...
    if (isset($_GET['action'])) {
        ...
        switch ($action) {
            case 'publish' :
                $result = $model->publish('articles', $all_id);
                break;

            case 'unpublish' :
                $result = $model->unpublish('articles', $all_id);
                break;

            case 'delete' :
                $result = $model->Wipe('articles', $all_id);
                if ($result) {
                    $ids_relations = "";
                    $ids_relation = $model->Get('articles_relation_tags', 'id', " WHERE article_id IN (" . $all_id . ") ");
                    foreach ($ids_relation as $ddd) {
                        $ids_relations .= ',' . $ddd['id'];
                    }
                    $model->Wipe('articles_relation_tags', ltrim($ids_relations, ','));
                }
                break;
        }
        if ($result) {
