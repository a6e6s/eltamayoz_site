...
if (isset($_GET['id'])) {
    ...
    if (isset($_GET['action'])) {
        ...
        switch ($action) {
            case 'publish' :
                $result = $model->publish('comments', $all_id);
                ($result) ? $result = $model->readable('comments', $all_id) : null;
                break;

            case 'unpublish' :
                $result = $model->unpublish('comments', $all_id);
                ($result) ? $result = $model->readable('comments', $all_id) : null;
                break;

            case 'readable' :
                $result = $model->readable('comments', $all_id);
                break;

            case 'delete' :
                $result = $model->Wipe('comments', $all_id);
                break;
        }
        if ($result) {
