...
if (isset($_GET['id'])) {
    ...
    if (isset($_GET['action'])) {
        ...
        switch ($action) {
            case 'publish' :
                $result = $model->publish('languages', $all_id);
                break;

            case 'unpublish' :
                $result = $model->unpublish('languages', $all_id);
                break;
            
            case 'is_default' :
                $result = $model->Update_Default('languages',$all_id);
                break;

            case 'delete' :
                $result = $model->Wipe('languages', $all_id);
                break;
        }
        if ($result) {
