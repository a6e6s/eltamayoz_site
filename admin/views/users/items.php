 ...
 */
$model = new model();
$items = $model->Get('users', 'id,username,email,status,avatar,register_date,last_login,admin');
?>
        ...
        <div class="main-content-inner">
            ...
            <div class="page-content">
                <div class="page-header">
                    <h1 class="col-xs-12 col-sm-10"> Users
                        <small>
                            <i class="ace-icon fa fa-angle-double-right"></i>
                            List of Users
                        </small>
                    </h1>
                    <a href="<?php echo ADMIN_URL; ?>users/new_user" class="btn btn-success col-xs-12 col-sm-2 btn-sm">New </a>
                            ...
                            <table id="dynamic-table" class="table table-striped table-bordered table-hover">
                                <thead>
                                    <tr>
                                        ...
                                        <th class="center" width="20%">Type</th>
                                ...
                                <tbody>
                                    ...
                                    foreach ($items as $item) {
                                        ...
                                        $summary_username = $model->Cut_Words($username, 0, 55, 'yes');
                                        $type = (isset($item['admin']) && $item['admin'] == '2') ? 'Admin' : 'User';
                                        $email = (isset($item['email'])) ? $item['email'] : '';
                                        $_register_date = (isset($item['register_date']) && $item['register_date'] != 0) ? date('d-m-Y   h:i A', $item['register_date']) : 'No Date';
                                        $_last_login = (isset($item['last_login']) && $item['last_login'] != 0) ? date('d-m-Y   h:i A', $item['last_login']) : 'Not Login';
                                        ?>
                                        <tr>
                                            <td class="center">
                                                <label class="pos-rel">
                                            ...
                                            <td>
                                                <a href="<?php echo ADMIN_URL . 'users/profile/' . $item['id']; ?>"><?php echo $summary_username; ?></a>
                                                <br/><small class="grey"><strong>Register:</strong> <?php echo $_register_date; ?></small><br/>
                                                <small class="grey"><strong>Last Login:</strong> <?php echo $_last_login; ?></small> 
                                            </td>
                                            <td class="center"><?php echo $email; ?></td>
                                            <td class="center"><?php echo $type; ?></td>
                                            <td>
                                                <div class="action-buttons">
                                                    <a href="#" value="<?php echo $item['id']; ?>" data-rel="tooltip" data-placement="top" title="<?php echo ($item['status'] == 0) ? 'Block' : 'Active'; ?>" onclick="action(this)" status="<?php echo ($item['status'] == 0) ? 'publish' : 'unpublish'; ?>" >
                                                        <i class="ace-icon fa fa- <?php echo ($item['status'] == 0) ? 'fa-minus-circle red' : 'fa-check green'; ?> bigger-130"></i>
                                                    </a>
                                                    <a href="<?php echo ADMIN_URL . 'users/edit_user/' . $item['id']; ?>" data-rel="tooltip" data-placement="top" title="Edit">
...
//    actions ........
                                                function action(anchor) {
                                                    ...
                                                    if (action == 'delete')
                                                    {
                                                        ...
                                                        if (x == true)
                                                        {
                                                            $.get("<?php echo ADMIN_URL ?>views/users/action.php?status=1&action=" + action + "&id=" + id, function(dd) {
                                                                location.reload();
                                                            });
                                                        }
                                                    } else
                                                    ...
                                                    $('.action').click(function() {
                                                        ...
                                                        } else
                                                        {
                                                            if (action == 'delete')
                                                            {
                                                                ...
                                                                if (x == true)
                                                                {
                                                                    $.get("<?php echo ADMIN_URL ?>views/users/action.php?action=" + action + "&id=" + id, function(dd) {
                                                                        location.reload();
                                                                    });
                                                                }
                                                            } else
