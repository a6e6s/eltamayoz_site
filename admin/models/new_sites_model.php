...
if (isset($_POST[$submit])) {
    ...
    foreach ($languages as $lang) {
        $site_name = $model->filtrate(str_replace(';', ' ', $_POST['site_name_' . $lang['alias']]));
        $site_closed_message = $model->filtrate(str_replace(';', ' ', $_POST['message_closed_' . $lang['alias']]));
        $site_logo = $model->filtrate(str_replace(';', ' ', $_POST['site_logo_' . $lang['alias']]));
        $site_desc = $model->filtrate(str_replace(';', ' ', $_POST['site_desc_' . $lang['alias']]));
        $site_keys = $model->filtrate(str_replace(';', ' ', $_POST['site_keys_' . $lang['alias']]));
    ...
    $post_alias = $model->filtrate(str_replace(';', ' ', $_POST['alias']));
    ...
    $_header_code = (isset($_POST['header_code']) && !empty($_POST['header_code'])) ? $_POST['header_code'] : '';
    $_footer_code = (isset($_POST['footer_code']) && !empty($_POST['footer_code'])) ? $_POST['footer_code'] : '';
    ...
    if (empty($errors)) {
        ...
        $result = $model->NewInsert('sites', $coulmns);
        if (is_numeric($result)) {
            ...
            $model->redirect_to(ADMIN_URL . 'sites/new_item');
        }
    } else {
        $all_error = "";
