...
if (is_array($categories)) {
    ...
    foreach ($categories as $cat) {
        if ($cat['alias'] == $alias) {
            $cat_id = $cat['id'];
            $cat_name_array = unserialize(base64_decode($cat['name']));
            $cat_name = $cat_name_array[$_SESSION['language_alias']];
            break;
        }
    }
    if ($cat_id == 0) {
...
$_page_title = '';
$_page_title .= $cat_name . ' - ' . $page_title . ' - ' . $_site_name;
//articles data
$per_page = $site_settings->per_page;
$total_count = $models->count_articles_by_category($cat_id);
...
$result = $models->articles_with_pagination_by_category($cat_id, $start, $per_page);
